#!/usr/bin/env bash
# ONE-SHOT N1 FULL-STACK BENCH runner (from the Mac). Resumes the Graviton2 box,
# syncs the local candle checkout (current branch = lambda-optimized/model-core,
# INCLUDING .git so the box can build model-core + origin/main), runs
# bench/fullstack_n1.sh box-side (builds both, bakes Q6packed-mc, benches
# model-core vs newmain vs llama over the thread tiers), fetches results, and
# ALWAYS stops the box on exit (cost control - EXIT trap).
#
# Box-side does TWO candle builds (~30-40 min) so MAXWAIT is generous. Robust to
# SSH drops: box runs under nohup + a DONE sentinel; this polls for it.
#
# Usage:
#   ./bench/run_fullstack.sh
#   THREADS="1 2 4 6" PP=512 TG=256 REPS=3 ./bench/run_fullstack.sh
#   KEEP_UP=1 ./bench/run_fullstack.sh
set -euo pipefail
...
# sync candle source (incl .git so the box can checkout model-core + origin/main;
# excl target + *.gguf - the box bakes Q6packed-mc itself) + bench scripts
...
RES="fullstack_results_$STAMP.txt"
DONE="fullstack_done_$STAMP"
say "launching box-side full-stack bench (nohup) -> ~/$RES"
"${SSH[@]}" "$SSH_USER@$DNS" "cat > ~/fullstack_box.sh" <<BOX
#!/usr/bin/env bash
set -uo pipefail
RES=~/$RES; DONE=~/$DONE
: > "\$RES"
THREADS="$THREADS" PP="$PP" TG="$TG" REPS="$REPS" \
  bash ~/ml-lambdas/bench/fullstack_n1.sh > "\$RES" 2>&1 || echo "(fullstack_n1.sh errored)" >> "\$RES"
touch "\$DONE"
BOX
"${SSH[@]}" "$SSH_USER@$DNS" "chmod +x ~/fullstack_box.sh && nohup bash ~/fullstack_box.sh >/dev/null 2>&1 & echo launched PID \$!"
