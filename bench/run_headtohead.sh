...
# bench/headtohead_n1.sh box-side (two builds, two bakes, benches explore vs
...
#   THREADS="1 2 4 6" PP=512 TG=256 REPS=3 ./bench/run_headtohead.sh
#   KEEP_UP=1 ./bench/run_headtohead.sh
set -euo pipefail
...
RES="headtohead_results_$STAMP.txt"
DONE="headtohead_done_$STAMP"
say "launching box-side head-to-head bench (nohup) -> ~/$RES"
"${SSH[@]}" "$SSH_USER@$DNS" "cat > ~/headtohead_box.sh" <<BOX
#!/usr/bin/env bash
set -uo pipefail
RES=~/$RES; DONE=~/$DONE
: > "\$RES"
THREADS="$THREADS" PP="$PP" TG="$TG" REPS="$REPS" \
  bash ~/ml-lambdas/bench/headtohead_n1.sh > "\$RES" 2>&1 || echo "(headtohead_n1.sh errored)" >> "\$RES"
touch "\$DONE"
BOX
"${SSH[@]}" "$SSH_USER@$DNS" "chmod +x ~/headtohead_box.sh && nohup bash ~/headtohead_box.sh >/dev/null 2>&1 & echo launched PID \$!"
