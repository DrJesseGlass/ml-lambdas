...
# bench/ropetg_n1.sh box-side (two builds, two bakes, benches explore vs
...
# Usage:
#   ./bench/run_ropetg.sh
#   THREADS="1 2 4 6" PP=512 TG=256 REPS=3 ./bench/run_ropetg.sh
#   KEEP_UP=1 ./bench/run_ropetg.sh
set -euo pipefail
...
RES="ropetg_results_$STAMP.txt"
DONE="ropetg_done_$STAMP"
say "launching box-side rope-tg bench (nohup) -> ~/$RES"
"${SSH[@]}" "$SSH_USER@$DNS" "cat > ~/ropetg_box.sh" <<BOX
#!/usr/bin/env bash
set -uo pipefail
RES=~/$RES; DONE=~/$DONE
: > "\$RES"
bash ~/ml-lambdas/bench/ropetg_n1.sh > "\$RES" 2>&1 || echo "(ropetg_n1.sh errored)" >> "\$RES"
touch "\$DONE"
BOX
"${SSH[@]}" "$SSH_USER@$DNS" "chmod +x ~/ropetg_box.sh && nohup bash ~/ropetg_box.sh >/dev/null 2>&1 & echo launched PID \$!"
