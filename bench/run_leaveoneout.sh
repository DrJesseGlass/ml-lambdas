...
# bench/leaveoneout_n1.sh box-side (two builds, two bakes, benches explore vs
...
#
# Usage:
#   ./bench/run_leaveoneout.sh
#   THREADS="1 2 4 6" PP=512 TG=256 REPS=3 ./bench/run_leaveoneout.sh
#   KEEP_UP=1 ./bench/run_leaveoneout.sh
set -euo pipefail
...
RES="leaveoneout_results_$STAMP.txt"
DONE="leaveoneout_done_$STAMP"
say "launching box-side leave-one-out bench (nohup) -> ~/$RES"
"${SSH[@]}" "$SSH_USER@$DNS" "cat > ~/leaveoneout_box.sh" <<BOX
#!/usr/bin/env bash
set -uo pipefail
RES=~/$RES; DONE=~/$DONE
: > "\$RES"
THREADS="$THREADS" PP="$PP" TG="$TG" REPS="$REPS" \
  bash ~/ml-lambdas/bench/leaveoneout_n1.sh > "\$RES" 2>&1 || echo "(leaveoneout_n1.sh errored)" >> "\$RES"
touch "\$DONE"
BOX
"${SSH[@]}" "$SSH_USER@$DNS" "chmod +x ~/leaveoneout_box.sh && nohup bash ~/leaveoneout_box.sh >/dev/null 2>&1 & echo launched PID \$!"
